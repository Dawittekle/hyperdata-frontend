...
interface BaseDataActionCellProps {
  renderDelete: (options: {
    isOpen: boolean;
    onClose: () => void;
  }) => ReactNode;
  renderEdit: (options: {
...
export function BaseDataActionCell({
  renderDelete,
  renderEdit,
}: BaseDataActionCellProps) {
  const [isEditOpen, setIsEditOpen] = useState(false);
  const [isDeleteOpen, setIsDeleteOpen] = useState(false);

  return (
    <>
      <button onClick={() => setIsEditOpen(true)} aria-label="Edit record">
        <svg
          width="33"
          height="28"
          viewBox="0 0 33 28"
      ...
      {renderEdit({ isOpen: isEditOpen, onClose: () => setIsEditOpen(false) })}
      {renderDelete({
        isOpen: isDeleteOpen,
